...
astronvim.default_compile_path = stdpath "data" .. "/packer/packer_compiled.lua"
...
function astronvim.initialize_packer()
  ...
  if not packer_avail then
    ...
    local packer_bootstrap = vim.fn.system({'git', 'clone', '--depth', '1', 'https://github.com/wbthomason/packer.nvim', packer_path})
...
function astronvim.toggle_term_cmd(term_details, close_on_exit)
  ...
  if type(term_details) == "string" then
    term_details = {
      ...
      on_exit = function(t, job, exit_code, name)
        -- vim.api.nvim_echo({'On_exit called with exit_code '})
        -- vim.api.nvim_echo(exit_code)
        if exit_code == 0 then
          t:shutdown()
        end
      end,
